...
| Centos       | centos 7.6                     |
+--------------+--------------------------------+
| Kernel       | 4.4.224-1.el7.elrepo.x86\_64   |
+--------------+--------------------------------+
| kubernetes   | v1.16.9                        |
+--------------+--------------------------------+
| kubespray    | v2.12.6                        |
...
| 主机名   | IP         | 角色      | 安装软件                                    |
+==========+============+===========+=============================================+
| i1-maste | 172.16.60. | k8s-Maste | ansible kubespray etcd ingress-nginx calico |
| r-1      | 178        | r01       | kube-apiserver kube-controller-manager      |
|          |            |           | kube-proxy kube-scheduler nodelocaldns      |
|          |            |           | node-exporte kubelet                        |
+----------+------------+-----------+---------------------------------------------+
| i1-worke | 172.16.60. | k8s-node0 | calico kube-proxy nginx-proxy nodelocaldns  |
| r-1      | 226        | 1         | node-exporte kubelet                        |
+----------+------------+-----------+---------------------------------------------+
| i1-worke | 172.16.60. | k8s-node0 | calico kube-proxy nginx-proxy nodelocaldns  |
| r-2      | 9          | 2         | node-exporte kubelet                        |
+----------+------------+-----------+---------------------------------------------+
...
`gcr.io/google\_containers <http://gcr.io/google_containers>`__ 和
