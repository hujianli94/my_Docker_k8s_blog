...
.. code:: shell
    ...
    https://blog.csdn.net/qq\_39680564/article/details/107238009
...
创建一个\ ``giteeGO-zk-activeMQ``\ 作为父\ ``Chart``\ 。

.. code:: bash

    $ helm create giteeGO-zk-activeMQ
...
``NOTES.txt``\ ：chart的 “帮助文本”。这会在用户运行helm
install时显示给用户。
...
创建\ ``activeMQ``\ 消息队列和一个\ ``zookeeper``\ 微服务，作为子\ ``Chart``\ 。

.. code:: shell

    $ helm create activeMQ \
    > && helm create zookeeper
    Creating activeMQ
    Creating zookeeper

现在当前目录我们一共有3个待调试的\ ``Chart``\ 。

::

    giteeGO-zk-activeMQ              ---父chart
    activeMQ                    ---子chart 
    zookeeper                   ---子chart
